...
parser.add_argument('--sw', dest='sensitive_window', default=DEFAULT_SENSITIVE_WINDOW,
                    help='Sensitivity window in seconds.', type=int)
parser.add_argument('--sv', dest='sensitive_threshold', default=DEFAULT_SENSITIVE_THRESHOLD,
                    help='Sensitivity threshold in milliwatts.', type=int)
parser.add_argument('-f', '--file', help='Output file to store the results.',
                    default="../logfiles/power_measurement.json")
parser.add_argument('--gpu_id', help='Which GPU to perform tests on.', default=0, type=int)
parser.add_argument('--recording_interval',
                    help='Values will be recorded if this amount of time(ms) has passed even though they don\'t change.',
                    default=DEFAULT_RECORDING_INTERVAL, type=int)
parser.add_argument('--timeout', help='This is the max amount of time in seconds that the process will wait \
    for power to be stabilized. If it doesn\'t average power consumption will be computed.', default=DEFAULT_TIMEOUT,
                    type=int)


def measure(conn, handle, sensitive_window=DEFAULT_SENSITIVE_WINDOW, recording_interval=DEFAULT_RECORDING_INTERVAL,
...
def simulate_shape(parent_conn, depth, width, batch_sizes, batches, input_dim, num_classes, model_class=None,
                   device=None):
    ...
    for batch_size in reversed(batch_sizes):
        ...
        if "power_not_stable" == measurement_status:
            ...
            fw = np.mean([to_microseconds(run['finish'] - run['start']) / 1000.0 for run in
                          runs[-last_n_runs:]])  # converted to milliseconds
            ...
            fw = np.mean([to_microseconds(run['finish'] - run['start']) / 1000.0 for run in
                          runs[-10:]])  # converted to milliseconds
            batch_results[batch_size] = {
                'stable_power': power,
                'stable_fw': fw,
                'energy': power * fw  # joules
...
def simulate(gpu_id, depths, widths, batch_sizes, input_dim, num_classes, log_dir=None, log_file=None,
             sensitive_window=DEFAULT_SENSITIVE_WINDOW, recording_interval=DEFAULT_RECORDING_INTERVAL,
             timeout=DEFAULT_TIMEOUT):
    parent_conn, child_conn = Pipe()
    recorder_process = Process(target=record_energy,
                               args=(child_conn, gpu_id, sensitive_window, recording_interval, timeout))
    ...
    for test_width in widths:
        for test_depth in depths:
            new_shape_result = simulate_shape(parent_conn, test_depth, test_width, batch_sizes, batches, input_dim,
                                              num_classes)
...
def main():
    ...
    simulation = None
    if opt.simulate:
        simulation = simulate(opt.gpu_id, depths, widths, batch_sizes, input_dim, opt.num_classes, log_file=opt.file,
                              sensitive_window=opt.sensitive_window, recording_interval=opt.recording_interval,
                              timeout=opt.timeout)
