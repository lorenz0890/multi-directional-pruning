...
def truncated_normal_variance_scaling(tensor: Tensor, gain: float = 1.) -> Tensor:
    fan_in, fan_out = _calculate_fan_in_and_fan_out(tensor)
    std = math.sqrt(gain / float(fan_in))
    a = 0 - math.sqrt((3 * gain) / float(fan_in))
    b = 0 + math.sqrt((3 * gain) / float(fan_in))
    return _no_grad_trunc_normal_(tensor, 0., std, a, b)
