...
class MCGDTopK:
    def __init__(self, model, train_loader, test_loader, config):
        ...
        self.gradient_diversity = MonteCarloGDTopKGradients(config.get('MCGDTOPK', 'lb', int), config.get('MCGDTOPK', 'k', int),config.get('MCGDTOPK', 'se', int) , model)

    def dispatch(self):
        torch.manual_seed(self.config.get('OTHER', 'seed', int))
        optimizer = Adam(self.model.parameters(), lr=self.config.get('MCGDTOPK', 'lr', float),
                              eps=self.config.get('MCGDTOPK', 'adam_eps', float))
    ...
    def __train(self, config, model, device, train_loader, test_loader, optimizer):
        for epoch in range(config.get('MCGDTOPK', 'epochs', int)):
