...
from gd import GradientDiversity, GradientDiversityTopKGradients, MonteCarloGDTopKGradients
...
class MCGDTopKACDK:
    def __init__(self, model, train_loader, test_loader, config):
        ...
        self.gradient_diversity = MonteCarloGDTopKGradients(config.get('MCGDTOPKACDK', 'lb', int), config.get('MCGDTOPKACDK', 'k', int),config.get('MCGDTOPKACDK', 'se', int) , model)
        self.ac = config.get('MCGDTOPKACDK', 'ac', int)
        self.loss_log = []
        self.loss_m_log = []
        self.loss_ratio_log = []
        self.k_batch_map = {}
        self.k_log = []
        self.k = config.get('MCGDTOPKACDK', 'k', int)
        self.k_max = config.get('MCGDTOPKACDK', 'k', int)
        self.accum_steps_log = []

    def dispatch(self):
        torch.manual_seed(self.config.get('OTHER', 'seed', int))
        optimizer = Adam(self.model.parameters(), lr=self.config.get('MCGDTOPKACDK', 'lr', float),
                              eps=self.config.get('MCGDTOPKACDK', 'adam_eps', float))
    ...
    def __train(self, config, model, device, train_loader, test_loader, optimizer):
        for epoch in range(config.get('MCGDTOPKACDK', 'epochs', int)):
            ...
            for batch_idx, (data, target) in enumerate(tqdm(train_loader)):
                ...
                loss = loss / config.get('MCGDTOPKACDK', 'ac', int)
                loss.backward()
                ...
                loss_m = sum(self.loss_log[-config.get('MCGDTOPKACDK', 'lb', int):]) / len(self.loss_log[-config.get('MCGDTOPKACDK', 'lb', int):])
                self.loss_m_log.append(loss_m)
                self.loss_ratio_log.append(loss / loss_m)
                if batch_idx not in self.k_batch_map:
                    self.k_batch_map[batch_idx] = []
                if loss > loss_m and self.k > 0:
                    ...
                    self.ac = min(config.get('MCGDTOPKACDK', 'lb', int) - batch_idx % config.get('MCGDTOPKACDK', 'lb', int), self.ac *1.1)
                else:
                    self.k_batch_map[batch_idx].append(self.k)
                self.accum_steps_log.append(self.ac)
                self.k = random.choice(self.k_batch_map[batch_idx][-config.get('MCGDTOPKACDK', 'lb', int):])
